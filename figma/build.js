(async () => {
  try {
    ...
    const primitivesCollection = figma.variables.createVariableCollection(
      "🎯 Primitives"
    );
    ...
    // 4b. THEME collection with 5 modes
    const themeCollection = figma.variables.createVariableCollection(
      "🌈 Theme"
    );
    themeCollection.renameMode(themeCollection.defaultModeId, "wepdf");

    const modeIds = { wepdf: themeCollection.defaultModeId };
    for (const brandName of ["trip-sorted", "checkin", "topup", "admin"]) {
      modeIds[brandName] = themeCollection.addMode(brandName);
    }

    // Create theme variables and set per-mode values
    ...
    for (const tokenName of themeTokenNames) {
      ...
      for (const brandName of Object.keys(THEMES)) {
        const hslStr = THEMES[brandName][tokenName];
        const color = hsl(hslStr);
        variable.setValueForMode(modeIds[brandName], color);
      }
    }

    // radius/base float variable
    ...
    for (const brandName of Object.keys(THEMES)) {
      radiusVariable.setValueForMode(
        modeIds[brandName],
        THEMES[brandName]["radius-base"]
      );
    }

    // 4c. SCALE collection
    const scaleCollection = figma.variables.createVariableCollection(
      "📐 Scale"
    );
    ...
    for (const title of grayTitles) {
      ...
      if (variable) {
        tile.setBoundVariable("fills", variable);
      }
    ...
    for (const [brandKey, brandData] of Object.entries(brandInfo)) {
      ...
      // Set the column to the specific theme mode
      brandColumn.setExplicitVariableModeForCollection(themeCollection, modeIds[brandKey]);
      ...
      taglineText.fills = [
        {
          ...
          boundVariables: {
            color: figma.variables
              .getLocalVariables()
              .find((v) => v.name === "muted-foreground"),
          },
        },
      ];
      ...
      for (const swatchName of swatchNames) {
        ...
        const colorVar = figma.variables
          .getLocalVariables()
          .find((v) => v.name === swatchName);
        if (colorVar) {
          swatch.setBoundVariable("fills", colorVar);
        }
      ...
      if (primaryVar) {
        buttonPreview.setBoundVariable("fills", primaryVar);
      }
      ...
      buttonText.fills = [
        {
          ...
          boundVariables: {
            color: figma.variables
              .getLocalVariables()
              .find((v) => v.name === "primary-foreground"),
          },
        },
      ];
      ...
      const cardVar = figma.variables
        .getLocalVariables()
        .find((v) => v.name === "card");
      if (cardVar) {
        cardPreview.setBoundVariable("fills", cardVar);
      }

      const cardBorder = figma.variables
        .getLocalVariables()
        .find((v) => v.name === "border");
      if (cardBorder) {
        cardPreview.setBoundVariable("strokes", cardBorder);
      }
      ...
      for (const size of componentSpecs.button.variants.size) {
        for (const fullWidth of componentSpecs.button.variants.fullWidth) {
          ...
          if (
            ...
            const fillColorVar = figma.variables
              .getLocalVariables()
              .find((v) => v.name === fillVar);
            if (fillColorVar) {
              component.setBoundVariable("fills", fillColorVar);
            }
          } else if (variant === "outline") {
            component.fills = [];
            const borderVar = figma.variables
              .getLocalVariables()
              .find((v) => v.name === "border");
            if (borderVar) {
              component.setBoundVariable("strokes", borderVar);
            }
          ...
          const textVar2 = figma.variables
            .getLocalVariables()
            .find((v) => v.name === textVar);
          if (textVar2) {
            buttonText.setBoundVariable("fills", textVar2);
          }
    ...
    const inputVar = figma.variables
      .getLocalVariables()
      .find((v) => v.name === "input");
    if (inputVar) {
      inputComponent.setBoundVariable("fills", inputVar);
    }
    ...
    const borderVar = figma.variables
      ...
      .find((v) => v.name === "border");
    if (borderVar) {
      inputComponent.setBoundVariable("strokes", borderVar);
    }
    ...
    const mutedVar = figma.variables
      .getLocalVariables()
      .find((v) => v.name === "muted-foreground");
    if (mutedVar) {
      inputText.setBoundVariable("fills", mutedVar);
    }
    ...
    const cardFillVar = figma.variables
      .getLocalVariables()
      .find((v) => v.name === "card");
    if (cardFillVar) {
      cardComponent.setBoundVariable("fills", cardFillVar);
    }

    const cardBorderVar = figma.variables
      .getLocalVariables()
      .find((v) => v.name === "border");
    if (cardBorderVar) {
      cardComponent.setBoundVariable("strokes", cardBorderVar);
    }
    ...
    const cardFgVar = figma.variables
      .getLocalVariables()
      .find((v) => v.name === "card-foreground");
    if (cardFgVar) {
      cardHeaderTitle.setBoundVariable("fills", cardFgVar);
    }
    ...
    if (mutedVar) {
      cardHeaderDesc.setBoundVariable("fills", mutedVar);
    }
    ...
    if (cardFgVar) {
      cardContent.setBoundVariable("fills", cardFgVar);
    }
    ...
    for (const badgeVariant of componentSpecs.badge.variants.variant) {
      ...
      const badgeFill = figma.variables
        .getLocalVariables()
        .find((v) => v.name === badgeFillVar);
      if (badgeFill) {
        badge.setBoundVariable("fills", badgeFill);
      }

      if (badgeVariant === "outline") {
        const badgeBorder = figma.variables
          .getLocalVariables()
          .find((v) => v.name === "border");
        if (badgeBorder) {
          badge.setBoundVariable("strokes", badgeBorder);
        }
      ...
      const badgeTextVarObj = figma.variables
        .getLocalVariables()
        .find((v) => v.name === badgeTextVar);
      if (badgeTextVarObj) {
        badgeText.setBoundVariable("fills", badgeTextVarObj);
      }
    ...
    for (const [brandKey, _] of Object.entries(brandInfo)) {
      ...
      // Set to the specific brand mode
      tile.setExplicitVariableModeForCollection(themeCollection, modeIds[brandKey]);
      ...
      const fgVar = figma.variables
        .getLocalVariables()
        .find((v) => v.name === "foreground");
      if (fgVar) {
        tileName.setBoundVariable("fills", fgVar);
      }
      tile.appendChild(tileName);

      // Bind card background
      const cardVar2 = figma.variables
        .getLocalVariables()
        .find((v) => v.name === "card");
      if (cardVar2) {
        tile.setBoundVariable("fills", cardVar2);
      }
      ...
      for (const swatchColor of ["primary", "accent", "secondary"]) {
        ...
        const swatchVar = figma.variables
          .getLocalVariables()
          .find((v) => v.name === swatchColor);
        if (swatchVar) {
          swatch.setBoundVariable("fills", swatchVar);
        }
