# Airport Check-in — SEO + PageSpeed Fix Plan

_Generated 2026-04-21T08:59:44+00:00 · [audit report](../products/airport_checkin.md) · [dashboard](../products/airport_checkin.jsx)_

## Sprint summary

- **Total tickets:** 10 (6 in-place, 4 architectural)
- **By priority:** P0=0, P1=6, P2=4
- **By owner:** DevOps=1, Frontend=9
- **Estimated total lift:** +80 score points (current: SEO 40.2, Perf 60.5)
...
#### `AIRPORT_CHECKIN-FIX-001` — sitemap.xml — Generate a sitemap (Next.js app router supports `sitemap.ts`).
...
**Affected**
  ...
  - `aicheckin.vercel.app`
  - `direct-check-in.my-trip-online.com`
...
#### `AIRPORT_CHECKIN-FIX-002` — canonical link — Emit a self-referential `<link rel='canonical'>` via metadata.
...
**Affected**
  ...
  - `aicheckin.vercel.app/`
  - `aicheckin.vercel.app/fr/`
  - `direct-check-in.my-trip-online.com/`
  - `direct-check-in.my-trip-online.com/fr/`
...
#### `AIRPORT_CHECKIN-FIX-003` — hreflang coverage — Emit hreflang for every locale route + x-default.
...
#### `AIRPORT_CHECKIN-FIX-004` — h1 count — Ensure exactly one h1 per route; demote duplicates to h2.

**Priority:** P2 · **Owner:** Frontend · **Effort:** 1h · **Est. lift:** +2 · **Area:** SEO · **Type:** fixable-in-place

**Problem**

Current: `2 h1 tags`. Target: `exactly 1 h1 per page`.
...
#### `AIRPORT_CHECKIN-FIX-005` — Open Graph — Emit full OG tags via Next.js metadata.openGraph.
...
#### `AIRPORT_CHECKIN-FIX-006` — JSON-LD — Inject JSON-LD for the page's primary entity type.
...
#### `AIRPORT_CHECKIN-ARCH-007` — Code-split and tree-shake the JS bundle
...
#### `AIRPORT_CHECKIN-ARCH-009` — Rewrite locale routing to emit hreflang + x-default centrally
...
#### `AIRPORT_CHECKIN-ARCH-010` — 301-migrate and sunset legacy domain https://direct-check-in.my-trip-online.com

**Priority:** P1 · **Owner:** DevOps · **Effort:** 2w · **Est. lift:** +15 · **Area:** SEO · **Type:** architectural

**Problem**

Maintaining both https://direct-check-in.my-trip-online.com and https://aicheckin.vercel.app splits link equity and confuses canonicalisation. The audit supports consolidating onto the clean domain.
...
#### `AIRPORT_CHECKIN-ARCH-008` — Migrate image pipeline to next/image with AVIF+WebP
...
2. Track status on the kanban board: `fix-seo-plans/airport_checkin.html` (open in a browser — status persists in localStorage).
3. Re-run `python scripts/run_audit.py --product airport_checkin` after shipping — the plan regenerates but IDs remain deterministic so tickets you've already filed can be updated in-place.
